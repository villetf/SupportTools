...
function Get-Options {
   ...
   do {
      ...
      Write-Host "Välkommen till SupportTools! Med SupportTools får du tillgång till ett flertal automatiserade verktyg för Organisationens supportfunktioner."`n
      ...
      switch ($selection) {
         ...
         2 {
            do {
               ...
               if ((Invoke-RestMethod -Uri "https://clientsystem.domain.se/Klienhanteringssystem/api/v2/user/validateName?name=$userToSearch" -UseDefaultCredentials).isValid) {
                  Write-Host "Användaren $userToSearch finns inte i Klienhanteringssystem. Kontrollera stavning och försök igen" -ForegroundColor Red
...
function Get-LockedAccounts {
   ...
   try {
      $userList = Search-ADAccount -LockedOut -SearchBase "OU=users,DC=users,DC=users,DC=se" | Where-Object {$_.Enabled -eq $true}
   }
   catch {
   ...
   if (Test-Connection "siem.domain.se" -Count 1 -TcpPort 443) {
      $lockoutInfo = Get-SIEMLockoutInfo
      $siemConnected = $true
   } else {
      Write-Warning "Kunde inte ansluta till SIEM. SIEM fungerar endast på trådat nät, och inte på VPN eller Wifi."
      Start-Sleep -Seconds 5
      $siemConnected = $false
   }
   ...
   foreach ($account in $userList) {
      try {
         $detailedList = Get-ADUser -Filter {SamAccountName -eq $account.SamAccountName} -SearchBase "OU=users,DC=users,DC=users,DC=se" -Properties *
      }
      catch {
...
function Get-LoggedOnComputers {
   ...
   try {
      $userInfo = Invoke-RestMethod -Uri "https://clientsystem.domain.se/Klienhanteringssystem/api/User?userName=$User" -UseDefaultCredentials
   }
   catch {
      if (($_ | Test-Json -ErrorAction SilentlyContinue) -and ($_ | ConvertFrom-Json).code -eq "ResourceNotFound") {
         Write-Error "Användaren $User finns inte i Klientsystem."
         Start-Sleep -Seconds 5
         return
      }
      Write-Error "Lista över datorer kunde inte hämtas från Klienhanteringssystem."
      Start-Sleep -Seconds 5
      return
   }

   $detailedList = Get-ADUser -Filter {SamAccountName -eq $User} -SearchBase "OU=users,DC=users,DC=users,DC=se" -Properties *
   
   if ($detailedList.LockedOut) {
      $lockedoutList = Get-SIEMLockoutInfo
   }
   ...
   foreach ($computer in $userInfo.clientLogins) {
      ...
      ## Hämtar info från servicedesk och lägger till i hashtablen
      try {
         $servicedeskInfo = Get-servicedeskAsset -Hostname $computer.name
         if ($servicedeskInfo) {
            $tempVar.value += "`nRum: $($servicedeskInfo.Rum), Våning: $($servicedeskInfo.Plan), Byggnad: $($servicedeskInfo.Byggnad), $($servicedeskInfo.Ort)"
            $tempvar.value += "`nDatortyp: $($servicedeskInfo.Licensform) dator, $($servicedeskInfo.Varutyp.replace('Dator ', ''))"
            $tempVar.value += "`nModell: $($servicedeskInfo.Fabrikat) $($servicedeskInfo.product.name)"
         } else {
...
function Get-ModuleVersion {
   ...
   try {
      $newestVersion = Invoke-RestMethod -Uri "https://serverx.domain.se/supporttools/stversion.txt" -Method Get
   }
   catch {
...
## Funktion för att hämta lokal information från dator via servicedesk
function Get-LocalInformation {
   ...
   try {
      $localInformation = Invoke-RestMethod -Uri "https://clientsystem.domain.se/Klienhanteringssystem/api/v2/client/$($currentComputer.name)/localInformation" -UseDefaultCredentials -ConnectionTimeoutSeconds 30
   }
   catch {
...
"@
   ...
   $assetId = (Invoke-RestMethod -Uri "https://servicedesk.domain.se/api/v3/assets" -Method Get -Body $data -Headers $technician_Key -ContentType "application/x-www-form-urlencoded").assets.id

   ## Gör anrop för att hämta info om asset baserat på ID
   $response = Invoke-RestMethod -Uri "https://servicedesk.domain.se/api/v3/assets/$assetId" -Method get -Headers $technician_Key
...
## Funktion för att hämta utelåsta konton från SIEM
function Get-SIEMLockoutInfo {

   $global:cred = Get-SIEMAuthentication
   $dqlQuery = "select%20categoryname(casc%20LIMIT%201000%20last%20120%20minutes"
   $id = (Invoke-RestMethod -Uri "https://siem.domain.se/api/searches?query=$dqlQuery" -Method Post -Headers @{
      'Version'       = '1.0'
      'Accept'        = 'application/json'
   } -Authentication Basic -Credential $cred).search_id

   do {
      $queryStatus = (Invoke-RestMethod -Uri "https://siem.domain.se/api/searches/$id" -Method GET -Headers @{
         'Version'       = '1.0'
         'Accept'        = 'application/json'
   ...
   $lockEvents = (Invoke-RestMethod -Uri "https://siem.domain.se/api/searches/$id/results" -Method GET -Headers @{
      'Range'         = 'items=0-49'
      'Version'       = '1.0'
      'Accept'        = 'application/json'
...
## Funktion för att fråga om användaruppgifter till SIEM
function Get-SIEMAuthentication {
   ...
   do {
      try {
         Invoke-RestMethod -Uri "https://siem.domain.se/api/saved_searches/3339" -Method GET -Headers @{
            'Version'       = '1.0'
            'Accept'        = 'application/json'
...
function Send-Log {
   ...
   try {
      Invoke-RestMethod -Uri "https://serverx.domain.se/api/supporttools/log" -Method Post -ContentType "application/json" -Body ($data | ConvertTo-Json)   
   }
   catch {
      return
   }
...
function Update-SupportTools {
   ...
   try {
      Invoke-WebRequest -Uri "https://serverx.domain.se/supporttools/SupportTools.zip" -OutFile "$env:LocalAppData\SupportTools\SupportTools.zip"
   }
   catch {
